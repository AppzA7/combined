...
def view_courier(request, loc_from,loc_to):
    ...
    for items in Courier.objects.filter(location_to = loc_to,location_from=loc_from):
        json_data[str(i)] = ''
        json_data[str(i)] = json_data[str(i)] + str(items.weight) + ':' + str(items.height) + ':' + str(items.width) + ':' + str(items.length) + ':' + items.location_to + ':' + items.location_from
        print json_data[str(i)]
        i = i+1
